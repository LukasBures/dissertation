...
sys.path.append("/home/lukas/PycharmProjects/Dissertation/_segmentation/")
from _segmentation.semantic_segmentation_NVIDIA import train as nvidia_train
...
confs = {
    ...
    "my_superpoint": {
        "output": "feats-superpoint-n8192-rmax1600",
        "model": {
            "name": "superpoint",
            "nms_radius": 3,
            "max_keypoints": 8192,
...
"""
segmentation_confs = {
    "nvidia_segmentation_default": {
        "method": "nvidia",
        "filter_classes": -1,
    },
    "deep_lab_v3_plus_default": {
...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.conf.resize_max and (self.conf.resize_force or max(w, h) > self.conf.resize_max):
...
@torch.no_grad()
def main(conf, segmentation_conf, image_dir, export_dir, as_half=False):
    logging.info("Extracting local features with configuration:" f"\n{pprint.pformat(conf)}")
    logging.info("Segmenting local features with configuration:" f"\n{pprint.pformat(segmentation_conf)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--segmentation_conf", type=str, default="nvidia_segmentation_default", choices=list(segmentation_confs.keys())
    )
    parser.add_argument("--image_dir", type=Path, required=True)
    parser.add_argument("--export_dir", type=Path, required=True)
    parser.add_argument("--as_half", action="store_true")
    args = parser.parse_args()
    main(confs[args.conf], segmentation_confs[args.segmentation_confs], args.image_dir, args.export_dir, args.as_half)
