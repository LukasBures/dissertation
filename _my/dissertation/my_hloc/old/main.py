...
import torch
from hloc import colmap_from_nvm, extract_features, localize_sfm, match_features, pairs_from_covisibility, triangulation
...
parser = argparse.ArgumentParser(description="main running file")
...
parser.add_argument(
    "--images", type=str, default="/home/lukas/PycharmProjects/Dissertation/datasets/aachen/images/images_upright/"
)
parser.add_argument(
    "--pairs", type=str, default="/home/lukas/PycharmProjects/Dissertation/HierarchicalLocalization/pairs/aachen/"
)
...
parser.add_argument("--netvlad", type=int, default=50, help="top N netvlad")
parser.add_argument(
    ...
    const=True,
    default=False,
    help="covisibility clustering in localize_sfm.py",
)
...
logx.logxx.initialize(logdir=args.result_dir, tensorboard=True, hparams=vars(args), global_rank=args.global_rank)
...
loc_pairs = pairs / f"pairs-query-netvlad{args.netvlad}.txt"  # top args.netvlad N retrieved by NetVLAD
reference_sfm = outputs / "sfm_superpoint+superglue"  # the SfM model we will build
results = outputs / f"Aachen_hloc_superpoint+superglue_netvlad{args.netvlad}.txt"  # the result file
...
triangulation.main(
    reference_sfm, outputs / "sfm_sift", images, sfm_pairs, feature_path, sfm_match_path, colmap_path="colmap"
)  # change if COLMAP is not in your PATH
