...
this_file_name=aachen_run_1080Ti_v2.sh
cd "$project_folder" || exit 1

# Aachen dataset settings.
used_dataset="aachen"
dataset_folder=/data512/datasets/aachen/
pipeline=/home/lukas/PycharmProjects/dissertation/_my/dissertation/my_hloc/pipelines/Aachen/pipeline_v2.py
...
time python "$output_root_folder"/"$folder_name"/code/pipeline_v2.py --dataset $dataset_folder --outputs $outputs_folder --num_covis $num_covis --num_loc $num_loc --retrieval_conf $retrieval_conf --feature_conf $feature_conf --matcher_conf $matcher_conf --gpu_number $selected_gpu 2>&1 | tee mylog.log
cd "$project_folder" || exit 1
