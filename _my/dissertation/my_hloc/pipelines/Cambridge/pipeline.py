...
from hloc import triangulation, localize_sfm, pairs_from_retrieval
...
def run_scene(images, gt_dir, outputs, results, num_covis, num_loc):
    ...
    create_query_list_with_intrinsics(gt_dir/"empty_all", query_list, test_list, ext=".txt", image_dir=images)
    ...
    triangulation.main(ref_sfm, ref_sfm_scaled, images, sfm_pairs, features, sfm_matches, colmap_path="colmap")

    loc_matches = match_features.main(matcher_conf, loc_pairs, feature_conf["output"], outputs)

    localize_sfm.main(ref_sfm, query_list, loc_pairs, features, loc_matches, results, covisibility_clustering=False, prepend_camera_name=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=Path, default="datasets/cambridge", help="Path to the dataset, default: %(default)s")
    parser.add_argument("--outputs", type=Path, default="outputs/cambridge", help="Path to the output directory, default: %(default)s")
    ...
    for scene in args.scenes:
        ...
        evaluate(gt_dirs/scene/"empty_all", all_results[scene], gt_dirs/scene/"list_query.txt", ext=".txt")
