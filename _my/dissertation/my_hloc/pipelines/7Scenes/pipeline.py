...
def run_scene(images, gt_dir, retrieval, outputs, results, num_covis, use_dense_depth, depth_dir=None):
    ...
    localize_sfm.main(ref_sfm, query_list, retrieval, features, loc_matches, results, covisibility_clustering=False, prepend_camera_name=True)
...
parser.add_argument("--dataset", type=Path, default="datasets/7scenes", help="Path to the dataset, default: %(default)s")
parser.add_argument("--outputs", type=Path, default="outputs/7scenes", help="Path to the output directory, default: %(default)s")
...
for scene in args.scenes:
    ...
    results = args.outputs/scene/"results_{}.txt".format("dense" if args.use_dense_depth else "sparse")
