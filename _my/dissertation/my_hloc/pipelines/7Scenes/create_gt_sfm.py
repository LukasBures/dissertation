...
def interpolate_depth(depth, kp):
    ...
    interp_nn = torch.nn.functional.grid_sample(depth, kp, align_corners=True, mode="nearest")[0, :, 0]
...
def correct_sfm_with_gt_depth(sfm_path, depth_folder_path, output_path):
    ...
    for imgid, img in tqdm(images.items()):
        ...
        scs = scene_coordinates(p2Ds[valids_backprojected], R_w2c, t_w2c, interp_depth[valids_backprojected], camera)
        invalid_p3D_ids = np.append(invalid_p3D_ids, p3D_ids[p3D_ids != -1][valids_projected][~valids_backprojected])
